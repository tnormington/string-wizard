...
export function RhythmPatterns() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-white">Rhythm Patterns</h2>
      ...
      {/* Playback controls */}
      <div className="flex flex-wrap items-center gap-4 p-4 rounded-xl bg-gray-800/60 border border-gray-700">
        {/* BPM control */}
        <div className="flex items-center gap-2">
          <label htmlFor="bpm-slider" className="text-sm text-gray-400 whitespace-nowrap">
            BPM
          </label>
          <input
            id="bpm-slider"
            ...
            onChange={(e) => {
              ...
              if (player.isPlaying && selectedPattern) {
                player.stop();
                // Slight delay so stop takes effect
                setTimeout(() => player.play(selectedPattern), 50);
              }
            }}
            className="w-24 accent-primary-500"
          />
          <span className="text-sm font-mono text-white w-10 text-right">{player.bpm}</span>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 rounded-lg bg-gray-800 border border-gray-700 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          />
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <select
            ...
            className="px-3 py-1.5 rounded-lg bg-gray-800 border border-gray-700 text-sm text-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-500"
          >
